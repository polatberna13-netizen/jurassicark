<template>
  <div class="bundle-card">
    <div class="bundle-header">
      <h2 class="title m-0">Boss Fight Tribute Extension</h2>
      <p class="subtitle m-0">Includes {{ safeItems.length }} artifacts</p>
    </div>
...
const safeItems = computed(() => (props.items || []).map(i => ({
  ...i,
...
const discountedTotal = computed(() => {
  const sum = safeItems.value.reduce((acc, it) => acc + toNumber(it.price) * it.qty, 0)
  return formatMoney(sum * (1 - DISCOUNT_RATE))
})
...
const addBundle = () => {
  ...
  add({
    itemId: 'boss-fight-tribute-extension',
    name: 'Boss Fight Tribute Extension',
    type: 'Bundle',
  ...
  success('Added Boss Fight Tribute Extension to cart')
}
</script>
...
.totals { display: inline-flex; align-items: baseline; gap: .5rem; }
